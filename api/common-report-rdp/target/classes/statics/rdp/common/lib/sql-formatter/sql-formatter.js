...
/***/ (function(module, exports, __webpack_require__) {
	...
	var Tokenizer = function () {
	    ...
	    function Tokenizer(cfg) {
	        ...
	        if(cfg.customField){
	        	this.CUSTOM_FIELD_REGEX = this.createcustomFieldRegex(cfg.customField);
	        }
	    }
	    ...
	    Tokenizer.prototype.createcustomFieldRegex = function createcustomFieldRegex(customField) {
	        var customFieldPattern = customField.join("|").replace(/ /g, "\\s+");
	        return new RegExp("^(" + customFieldPattern + ")", "");
	    };
	    ...
	    Tokenizer.prototype.getNextToken = function getNextToken(input, previousToken) {
	        return this.getWhitespaceToken(input)||this.getCustomFieldToken(input)|| this.getCommentToken(input) || this.getStringToken(input) || this.getOpenParenToken(input) || this.getCloseParenToken(input) || this.getPlaceholderToken(input) || this.getNumberToken(input) || this.getReservedWordToken(input, previousToken) || this.getWordToken(input) || this.getOperatorToken(input);
	    };

	    Tokenizer.prototype.getCustomFieldToken = function getCustomFieldToken(input){
	    	if(this.CUSTOM_FIELD_REGEX){
	    		return this.getTokenOnFirstMatch({
	    			input: input,
	    			type: "customFiledToken",
	    			regex: this.CUSTOM_FIELD_REGEX
	    		});
	    	}
	    }
