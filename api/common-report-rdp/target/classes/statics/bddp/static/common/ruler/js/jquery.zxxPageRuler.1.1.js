	...
	$.pageRuler = function (params) {
		...
		var f = {
			...
			ui: function () {
				...
				for (var i = 0; i < w; i += 1) {
					if (i % 50 === 0) {
						$('<span class="n" title="'+i+'px">' + i + '</span>').css("left", i ).appendTo(rh);
					}
				}
				//垂直标尺数值
				for (var i = 0; i < h; i += 1) {
					if (i % 50 === 0) {
						$('<span class="n" title="'+i+'px">' + i + '</span>').css("top", i).appendTo(rv);
					}
				}
			},
			ie6: function () {
			...
			dashh: function () {
				$(document).bind("mousemove", function (e) {
					if (dragFlag) {
						//如果可以拖拽
						//垂直虚线的左坐标
						doth.css("top", (e.pageY - $("#content").offset().top) / $("#content").data("zoom"));
					}
				});
			},
			//弹出框相关方法
		...
		$(document).mouseup(function (e) {
			...
			if (oDrag) {
				if (oDrag.hasClass("zxxRefLine_v")) {
					var v_l = (e.pageX - $("#content").offset().left) / $("#content").data("zoom");
					if (v_l < rv.width()) {
						v_l = -600;
						$(oDrag).remove();
					}else{
					...
					var v_t = (e.pageY - $("#content").offset().top) / $("#content").data("zoom");
					if (v_t < rh.height()) {
						v_t = -600;
						$(oDrag).remove();
					}else{
