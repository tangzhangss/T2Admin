...
(function ($, window, document, undefined) {
    ...
    var TheTable = function (opt) {
        ...
        this.columConfig = {
            ...
            //一级参数，默认配置
            this.defaults = {
                'laynum': 1, //需要自动生成的表格的层数 必填
                'startlay': 1,//从哪层开始.默认为1 即lvl=1的数据开始.顺序为从上往下.
    ...
    TheTable.prototype = {
        ...
        writeTableData: function () {
            if (leafList.length > 0) {
                ...
                $(this.options.tableid).find('#' + this.tablestr.prefix + this.tablestr.body).append(trall);
            }
        },
        /**
         * 生成表头对应的输入框
                ...
                if (item) {
                    ...
                    if (lvl == level) {
                        ...
                        if (isleaf == 'Y') {
                            ...
                            $this.options.laynum > lvl && optionList.push([$this.tablestr.rows, $this.options.laynum + $this.options.startlay  - lvl]);
        ...
        writeHeader: function (table, headerList) {
            if (headerList.length >= 0) {
                ...
                $(this.options.tableid).find('#' + this.tablestr.prefix + this.tablestr.head).append(headtr);
                //防止之前是隐藏状态，在这里进行一次显示
                table.show();
            }
        },
        ...
        showDescDiv: function (divin, divout, titlestr, desc, pagex, pagey) {
            ...
            divout.css({
                top: pagey + this.options.descConfig.pos_y,
                left: pagex + this.options.descConfig.pos_x
            }).show();
    ...
    TheAnaLyze.prototype = {
        ...
        getCompareClassConfig: function () {
            return {
                    'max': {'eq': this.options.compare.eqclass},
                    'min': {'eq': this.options.compare.eqclass},
                    'avg': {
                        'up': this.options.compare.upclass,
                        'down': this.options.compare.lowclass,
                        'eq': this.options.compare.eqclass
                    }
                }[this.options.anatype] || ''
